...
def gpu_mem_usage(gpu_index=0):
    ...
    if torch.cuda.is_available():
        ...
        mem_usage_bytes = memory_info.used
...
def launch_job_co(cfg_list, init_method, func, daemon=False):
    """
    Run 'func' on one or more GPUs, specified in cfg
    Args:
        cfg_list (CfgNode): configs. Details can be found in
            slowfast/config/defaults.py
        init_method (str): initialization method to launch the job with multiple
            devices.
        func (function): job to run on GPU(s)
        daemon (bool): The spawned processes’ daemon flag. If set to True,
            daemonic processes will be created
    """
    if cfg_list[0].NUM_GPUS > 1:
        torch.multiprocessing.spawn(
            mpu.run,
            nprocs=cfg_list[0].NUM_GPUS,
            args=(
                cfg_list[0].NUM_GPUS,
                func,
                init_method,
                cfg_list[0].SHARD_ID,
                cfg_list[0].NUM_SHARDS,
                cfg_list[0].DIST_BACKEND,
                cfg_list,
            ),
            daemon=daemon,
        )
    else:
        func(cfg_list=cfg_list)
