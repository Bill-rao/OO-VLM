...
def get_path_to_checkpoint(path_to_job, epoch, save_latest=False):
    """
...
def get_last_checkpoint(path_to_job, save_latest=False):
    """
...
def is_checkpoint_epoch(cfg, cur_epoch, multigrid_schedule=None, svae_last=False):
    """
    ...
    """
    if svae_last and cur_epoch + 1 == cfg.SOLVER.MAX_EPOCH:
        return True
...
def save_best_checkpoint(path_to_job, model, optimizer, loss_scaler, epoch, cfg, zip=True, name='best.pyth'):
    """
    Save a checkpoint.
    Args:
        model (model): model to save the weight to the checkpoint.
    ...
    # Write the checkpoint.
    path_to_checkpoint = os.path.join(path_to_job, name)
    with g_pathmgr.open(path_to_checkpoint, "wb") as f:
        if not zip:
            torch.save(checkpoint, f, _use_new_zipfile_serialization=False)
        else:
            torch.save(checkpoint, f)
    logger.info(f"Save best checkpoint at epoch-{epoch}!!!")
    return path_to_checkpoint
...
def load_train_checkpoint(cfg, model, optimizer, loss_scaler):
    ...
    """
    d = cfg.OUTPUT_DIR
    names = g_pathmgr.ls(d)
    names = [f for f in names if "best" in f]

    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(cfg.OUTPUT_DIR, save_latest=cfg.TRAIN.SAVE_LATEST)
        logger.info("Load from last checkpoint, {}.".format(last_checkpoint))
        checkpoint_epoch = load_checkpoint(
            last_checkpoint, model, loss_scaler, cfg.NUM_GPUS > 1, optimizer, epoch_reset=cfg.TRAIN.IS_RESET_EPOCH
        )
        start_epoch = checkpoint_epoch + 1
    elif cfg.TEST.TEST_BEST and g_pathmgr.exists(os.path.join(cfg.OUTPUT_DIR, 'best.pyth')):
        best_checkpoint = get_best_checkpoint(cfg.OUTPUT_DIR)
        logger.info("Load from best checkpoint file.")
        checkpoint_epoch = load_checkpoint(
            best_checkpoint, model, loss_scaler, cfg.NUM_GPUS > 1, optimizer, epoch_reset=cfg.TRAIN.IS_RESET_EPOCH
        )
        start_epoch = checkpoint_epoch + 1
    elif cfg.TEST.TEST_BEST and len(names):
        name = sorted(names)[-1]
        best_checkpoint = os.path.join(d, name)
        logger.info("Load from best checkpoint file.")
        checkpoint_epoch = load_checkpoint(
            best_checkpoint, model, loss_scaler, cfg.NUM_GPUS > 1, optimizer, epoch_reset=cfg.TRAIN.IS_RESET_EPOCH
        )
