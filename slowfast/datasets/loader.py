...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    if isinstance(dataset, torch.utils.data.IterableDataset):
        ...
        if (
            ...
            # Create a sampler for multi-process training
            # if not cfg.FUSION.ENABLE:  # NOTE: fusion 验证每个都走完整的数据集，不做数据集分割
            #     sampler = utils.create_sampler(dataset, shuffle, cfg)
            # else:
            #     sampler = None

            # if cfg.FUSION.ENABLE and split not in ["train"]:
            #     sampler = None
            # else:
            #     sampler = utils.create_sampler(dataset, shuffle, cfg)
            sampler = utils.create_sampler(dataset, shuffle, cfg)
...
def shuffle_dataset(loader, cur_epoch):
    ...
    # Check sample type-only RandomSampler(Single GPU) and DistributedSampler(Multi GPU) support
    # Sequential Sampler does not support
    if not isinstance(sampler, (RandomSampler, DistributedSampler)):
        logger.warning("Sampler type '{}' not supported".format(type(sampler)))
        return
