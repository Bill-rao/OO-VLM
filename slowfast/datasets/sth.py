...
class Sth(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        """
        if self.cfg.MODEL.MODEL_NAME == "Fusion":
            assert self.cfg.DATA.SKIP_IDS, "SKIP_IDS is empty"
        ...
        for item in tmp:
            ...
            # data filter
            if self.cfg.MODEL.MODEL_NAME == "Fusion":
                if int(path) in self.cfg.DATA.SKIP_IDS:
                    continue

            self._path_to_videos.append([path, num_frames])
            self._labels.append(label)
            self.video_ids.append(video_index)
        ...
        self.video_ids = list(
            chain.from_iterable([[x] * self._num_clips for x in self.video_ids])
        )
    ...
    def __getitem__(self, index):
        ...
        frames = torch.as_tensor(  # videoname_06frame
            utils.retry_load_images(
                [path_template.format(self._path_to_videos[index][0], frame) for frame in seq],
                self._num_retries,
            ))

        label = self._labels[index]
        video_index = self.video_ids[index]
        ...
        if self.aug:
            if self.cfg.AUG.NUM_SAMPLE > 1:
                ...
                for _ in range(self.cfg.AUG.NUM_SAMPLE):
                    ...
                    # assert index // self._num_clips == video_index, f"{index // self._num_clips} != {video_index}"
                    video_index_list.append(video_index)

                return frame_list, label_list, index_list, {'video_index': torch.tensor(video_index_list)}
        ...
        # assert index // self._num_clips == video_index, f"{index // self._num_clips} != {video_index}"
        return frames, label, index, {'video_index': torch.tensor(video_index)}
