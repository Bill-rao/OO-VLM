...
class GroupMultiScaleCrop(object):
    ...
    def __call__(self, img_w, img_h):

        # im_size = img_group[0].size
        # im_size = (img_w, img_h)

        crop_w, crop_h, offset_w, offset_h = self._sample_crop_size(img_w, img_h)
        ...
        return offset_h, offset_w, crop_h, crop_w
...
def horizontal_flip_with_label(prob, images, label, label_transforms, boxes=None):
    """
    Perform horizontal flip on the given images and corresponding boxes.
    Args:
        prob (float): probility to flip the images.
    ...
    if np.random.uniform() < prob:
        images = images.flip((-1))
        if label in label_transforms.keys():
    ...
    return images, label, flipped_boxes
